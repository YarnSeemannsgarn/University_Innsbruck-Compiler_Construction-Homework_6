...
identListType           : identList T_COLON type { $$ = new_node(IDENT_LIST_TYPE); $$->body[0] = $1; $$->body[1] = $3; }
                        ;
 
identList               : T_ID T_COMMA identList { $$ = new_node(IDENTIFIER); $$->identifier = $1; $$->next = $3; }
                        | T_ID { $$ = new_node(IDENTIFIER); $$->identifier = $1; }
                        ;
...
simpleType	        : T_INTEGER { $$ = new_node(SIMPLE_TYPE); $$->iValue = T_INTEGER; }
                        | T_REAL { $$ = new_node(SIMPLE_TYPE); $$->iValue = T_REAL; }
                        | T_BOOLEAN { $$ = new_node(SIMPLE_TYPE); $$->iValue = T_BOOLEAN; }
                        ;
...
assignStmt              : T_ID T_ASSIGNMENT expr { $$ = new_node(ASSIGN); $$->body[0] = new_node(IDENTIFIER); $$->body[0]->identifier = $1; $$->body[2] = $3; }
                        | T_ID T_LEFT_SQUARE_BRACKET expr T_RIGHT_SQUARE_BRACKET T_ASSIGNMENT expr { $$ = new_node(ASSIGN); $$->body[0] = new_node(IDENTIFIER); $$->body[0]->identifier = $1; $$->body[1] = $3; $$->body[2] = $6; }
                        ;
...
forStmt		        : T_FOR T_ID T_ASSIGNMENT expr toPart expr T_DO statement { $$ = new_node(FOR); $$->body[0] = new_node(IDENTIFIER); $$->body[0]->identifier = $2; $$->body[1] = $4; $$->body[2] = $5; $$->body[3] = $6; $$->body[4] = $8; }
                        ;
 
toPart		        : T_TO { $$ = new_node(FOR_TO_PART); $$->iValue = T_TO; }
		        | T_DOWNTO { $$ = new_node(FOR_TO_PART); $$->iValue = T_DOWNTO; }
                        ;
...
factor		        : number { $$ = $1; }
		        | T_FALSE { $$ = new_node(BOOL_CONST); $$->iValue = 0; }
		        | T_TRUE { $$ = new_node(BOOL_CONST); $$->iValue = 1; }
		        | T_ID { $$ = new_node(IDENTIFIER); $$->identifier = $1; }
                        | T_ID T_LEFT_SQUARE_BRACKET expr T_RIGHT_SQUARE_BRACKET { $$ = new_node(IDENTIFIER_SUBSCRIPT); $$->body[0] = new_node(IDENTIFIER); $$->body[0]->identifier = $1; $$->body[1] = $3; }	
		        ...
		        | T_STRING { $$ = new_node(STRING_CONST); $$->identifier = $1; }
                        ;

number                  : T_NUM_INT { $$ = new_node(INT_CONST); $$->iValue = $1; }
                        | T_NUM_REAL { $$ = new_node(REAL_CONST); $$->fValue = $1; }
                        ;
